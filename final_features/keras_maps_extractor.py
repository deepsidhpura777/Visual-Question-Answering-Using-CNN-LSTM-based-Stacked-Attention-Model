...
original_images_batches = pickle.load(open('images_batches_list.pkl','rb+'))
...
mega_onlyfiles_split = split_seq(mega_onlyfiles,50)  ####### 20 for val. Might need to change to 70
...
for split in mega_onlyfiles_split:
    ...
    for zip_file in split:
        ...
        for image_file in temp_files:
            ...
            im = cv2.resize(cv2.imread(img_path), (299,299)).astype(np.float32)
	...
	os.remove(mypath + '/' + zip_file)   ##### Commented for testing purposes
    ...
    temp_array = []
    temp_images = []
    #print 'batch_mappings:',batch_mappings
    for b in keys:
	...
	#print "b value:",b
        batch_index = int(b[b.find('_')+1:]) - 1
	#print "Batch_Index:",batch_index   #### batch_index to index the original_images_batches
	#print "img_batch:",img_batch
	#print "len img_batch:",len(img_batch)
        original_images = original_images_batches[batch_index] #### Should be 128
        for i in original_images:
            temp_index = img_batch.index(str(i))
